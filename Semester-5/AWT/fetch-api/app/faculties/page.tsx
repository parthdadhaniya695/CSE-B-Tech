...
async function Faculties() {
    const res = await fetch("https://6875d513814c0dfa6539907e.mockapi.io/faculties",{
        next: { revalidate: 0 },
    }); 
  ...
  return (
    ...
    <h2 className="text-4xl font-bold mb-8 text-gray-800 text-center">Faculty Directory</h2>
        ...
        >
          <div className="flex flex-col items-center text-center">
            <div className="w-24 h-24 bg-blue-100 text-blue-600 flex items-center justify-center rounded-full text-2xl font-semibold mb-4">
              {d.FacultyName.charAt(0)}
            </div>
            <h3 className="text-lg font-semibold text-gray-800">{d.FacultyName}</h3>
          </div>
        </Link>
      ))}
    </div>
